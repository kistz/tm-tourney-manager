...
spacetime delete tmservers-wqk3g || true
...
# Create project
spacetime call tmservers-wqk3g create_project "Double Elimination" "This is a beautiful project" '{"Tournament": {}}' '{"__timestamp_micros_since_unix_epoch__": 1777132984000000}' '{"__timestamp_micros_since_unix_epoch__": 1777233084000000}'

# Double elimination bracket
spacetime call tmservers-wqk3g competition_create "Double Elimination Bracket" 1 0

# Upper R1
# 8 matches, Match IDs 1-8
for i in {1..8}; do
  spacetime call tmservers-wqk3g match_create "Upper R1 M$i" 2 0
done

# Upper R2
# 4 matches, Match IDs 9-12
for i in {1..4}; do
  spacetime call tmservers-wqk3g match_create "Upper R2 M$i" 2 0
done
...
for match in {9..12}; do
  ...
  for target in {1..8}; do
    ...
    if [[ " ${winners[@]} " =~ " ${target} " ]]; then
      spacetime call tmservers-wqk3g connection_create \
        "{\"MatchV1\": $match }" \
        "{\"MatchV1\": $target }" \
        '{"Data": {}}'
...
for i in {1..4}; do
  spacetime call tmservers-wqk3g match_create "Lower R1 M$i" 2 0
done
...
for i in {1..4}; do
  spacetime call tmservers-wqk3g match_create "Lower R2 M$i" 2 0
done
...
for i in {1..2}; do
  spacetime call tmservers-wqk3g match_create "Upper R3 M$i" 2 0
done
...
for i in {1..2}; do
  spacetime call tmservers-wqk3g match_create "Lower R3 M$i" 2 0
done
...
for i in {1..2}; do
  spacetime call tmservers-wqk3g match_create "Lower R4 M$i" 2 0
done
...
# Upper Final
# 1 match, Match ID 27
spacetime call tmservers-wqk3g match_create "Upper Final" 2 0
...
# Lower Final
# 1 match, Match ID 28
spacetime call tmservers-wqk3g match_create "Lower Final" 2 0
...
# Consolidation Final
# 1 match, Match ID 29
spacetime call tmservers-wqk3g match_create "Consolidation Final" 2 0
...
# Grand Final
# 1 match, Match ID 30
spacetime call tmservers-wqk3g match_create "Grand Final" 2 0
